...
export default component$(() => {
	...
	return (
		<div>
			<div
				class={`fixed inset-0 overflow-hidden z-20 transition-opacity duration-500 ease-in-out ${
					appState.showCart ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
				}`}
			>
				<div class="absolute inset-0 overflow-hidden">
					<div
						class={`absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity duration-500 ease-in-out`}
						onClick$={() => (appState.showCart = false)}
					></div>

					<div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
						<div
							class={`w-screen max-w-md transform transition-transform duration-500 ease-in-out ${
								appState.showCart ? 'translate-x-0' : 'translate-x-full'
							}`}
						>
